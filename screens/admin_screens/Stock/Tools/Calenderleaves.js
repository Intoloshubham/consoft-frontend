...
const DatePicker = ({
  ...
  const getDateInfo = (givenDate) => {
   ...
   const getInitialDate = (selectedDate, initialDate, hasChanged) => {
    if (hasChanged) {
      return selectedDate;
    }
 ...
 const getYearArray = (selectedDate) => {
  ...
  const dateProp = (props, propName, componentName) => {
    if (!props[propName]) {
      return;
    }
