    ...
    if (response.data) {
      {
        response.data.map(async (ele, i) => {
          if (
            ...
            ele.verification_2 === user_id ||
            ele.started_by === user_id
          ) {
            let user_id = '';
    ...
    const renderItem = ({item}) => (
      <TouchableOpacity
        style={{
          ...
          elevation: 1,
          borderLeftWidth: 5,
          borderColor: item.final_verify_status
            ? COLORS.green
            : COLORS.yellow_500,
        }}
        onPress={() => {
      ...
      <Modal animationType="slide" transparent={true} visible={reportModal}>
        <View
          ...
          <View
            ...
            <View style={{flex: 1}}>{trackVerificationProcess()}</View>
          </View>
        </View>
      </Modal>
    );
